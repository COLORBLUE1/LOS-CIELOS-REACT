...
app.put("/api/tipos-de-parapente/:id", (req, res) => {
  ...
  let parapente = parapenteData.tipos_de_parapente.find(
    (p) => p.id === parseInt(id)
  );
  if (!parapente) {
...
app.delete("/api/tipos-de-parapente/:id", (req, res) => {
  const { id } = req.params;
  const index = parapenteData.tipos_de_parapente.findIndex(
    (p) => p.id === parseInt(id)
  );
...
app.put("/api/usuarios/:id", (req, res) => {
  ...
  const { id } = req.params;
  const query =
    "UPDATE credenciales SET email = ?, password = ?, rol = ? WHERE id = ?";
